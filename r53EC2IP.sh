...
MYNAME="amermachine"        		   # nflx
EC2_IP=`/usr/bin/curl -s http://169.254.169.254/latest/meta-data/public-ipv4 | cut -d ' ' -f 2`
...
logger "ROUTE53: Setting DNS A $MYNAME.$ZONE for $FQN $EC2_IP"
...
/usr/bin/cli53 rrcreate --replace $ZONE "$MYNAME 60 A $FQN"
